        ...
        <div class="fieldset mt-2 space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <input class="input w-full" id="holiday_name" name="holiday_name" type="text" placeholder="e.g. Christmas" value="{{ old('holiday_name', $holiday->name) }}" required />
            </div>
            <div class="space-y-1">
              <label class="fieldset-label" for="fixed_price">Holiday Price (USD)*</label>
              <label class="input w-full focus:outline-0">
                <input class="grow focus:outline-0" id="fixed_price" name="fixed_price" type="text" placeholder="e.g. 20" value="{{ old('fixed_price', $holiday->fixed_price) }}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required />
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-2">
                <input
                  ...
                  value="one_day"
                  {{ old('application_type', $holiday->application_type) === 'one_day' ? 'checked' : '' }}
              ...
              <div class="flex items-center gap-2">
                <input
                  ...
                  name="application_type"
                  value="period_days"
                  {{ old('application_type', $holiday->application_type) === 'period_days' ? 'checked' : '' }}
                  onchange="toggleDateFields()"
                  required
                />
          ...
          <div id="date_fields_container" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div id="one_day_field" class="space-y-1" style="display: {{ old('application_type', $holiday->application_type) === 'one_day' ? 'block' : 'none' }};">
              <label class="fieldset-label" for="holiday_date">Holiday Date*</label>
              <input class="input w-full" id="holiday_date" name="holiday_date" type="date" value="{{ old('holiday_date', $holiday->date ? $holiday->date->format('Y-m-d') : '') }}" />
            </div>

            <div id="period_days_fields" style="display: {{ old('application_type', $holiday->application_type) === 'period_days' ? 'grid' : 'none' }};" class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                <label class="fieldset-label" for="period_start_date">Start Date*</label>
                <input class="input w-full" id="period_start_date" name="holiday_date" type="date" value="{{ old('holiday_date', $holiday->date ? $holiday->date->format('Y-m-d') : '') }}" />
              </div>
              <div class="space-y-1">
                <label class="fieldset-label" for="end_date">End Date*</label>
                <input class="input w-full" id="end_date" name="end_date" type="date" value="{{ old('end_date', $holiday->end_date ? $holiday->end_date->format('Y-m-d') : '') }}" />
              </div>
            </div>
          </div>
